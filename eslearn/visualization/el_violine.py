...
class ViolinPlot(object):
    ...
    def plot(self, data, xlabel=None, ylabel=None, xlabelsize=10, ylabelsize=10, xticklabel=None, yticklabel=None):
        """Plot
        ...
        self.set_axis_style(xlabel, ylabel, xlabelsize, ylabelsize, xticklabel, yticklabel)
        plt.subplots_adjust(bottom=0.15, wspace=0.05)
        plt.show()

    def set_axis_style(self, xlabel, ylabel, xlabelsize, ylabelsize, xticklabel, yticklabel):
        self.ax.get_xaxis().set_tick_params(direction='out')
        self.ax.xaxis.set_ticks_position('bottom')
        if xticklabel:
            self.ax.set_xticks(np.arange(0, len(xticklabel) ))
            self.ax.set_xticklabels(xticklabel, rotation=90)
