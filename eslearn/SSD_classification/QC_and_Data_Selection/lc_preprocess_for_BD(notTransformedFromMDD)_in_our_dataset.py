...
# Inputs
matroot = r'D:\WorkStation_2018\WorkStation_CNN_Schizo\Data\SelectedFC_BDNottransformedFromMDD'  # all mat files directory
# scale_all_BD = 'D:\WorkStation_2018\WorkStation_CNN_Schizo\Scale\BD.txt'
# scale_MDD2BD = 'D:\WorkStation_2018\WorkStation_CNN_Schizo\Scale\抑郁转躁.txt'
...
# Filter BD that not transformed from MDD
# uid_all_BD = pd.read_csv(scale_all_BD,header=None)
# uid_MDD2BD = pd.read_csv(scale_MDD2BD,header=None)
# uid_BD_not_transformed_from_MDD = pd.DataFrame(list(set(uid_all_BD.iloc[:,0]).difference(set(uid_MDD2BD.iloc[:,0]))))
# uid_BD_not_transformed_from_MDD.to_csv(r'D:\WorkStation_2018\WorkStation_CNN_Schizo\Scale\BD_not_transformed_from_MDD.txt', header=False, index=False)
...
selected_diagnosis[selected_diagnosis==4] = 1
allmat_plus_label = pd.concat([selected_diagnosis, allmat],axis=1)
print(allmat_plus_label.iloc[:,0])
np.save(r'D:\WorkStation_2018\WorkStation_CNN_Schizo\Data\ML_data_npy\OurCenter_BDNottransformedFromMDD.npy',allmat_plus_label)