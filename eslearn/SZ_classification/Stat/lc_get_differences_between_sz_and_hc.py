# -*- coding: utf-8 -*-
"""
This script is used to extract covariance of medicated SSD and first episode unmedicated SSD, as well as their matched HC.
This script is also used to get the Cohen'd values.
"""
import sys
...
cov_medicated = pd.DataFrame(np.concatenate([cov_medicated_sp_hc_550, cov_206, cov_COBRE, cov_UCLA], axis=0), columns=header)
...
cov_medicated = pd.concat([cov_medicated['folder'], cov_all_sites_id, cov_medicated[['diagnosis', 'age', 'sex']]], axis=1)
                    
data_ssd_medicated = np.concatenate([features_our_center_484, features_206, features_UCAL, features_COBRE], axis=0)
label_medicated = cov_medicated['diagnosis']
...
#%% Get the difference
# Medicated
data_ssd_medicated = data_ssd_medicated[label_medicated == 1]
data_hc_medicated = data_ssd_medicated[label_medicated == 0]
cohen_medicated = CohenEffectSize(data_ssd_medicated, data_hc_medicated)

# First episode unmedicated in dataset1
data_ssd_firstepisodeunmed = feature_firstepisodeunmed[label_firstepisodeunmed == 1]
...
if is_save:
    sio.savemat(r'D:\WorkStation_2018\SZ_classification\Data\fc_medicated.mat', {'fc_medicated': data_ssd_medicated})
    sio.savemat(r'D:\WorkStation_2018\SZ_classification\Data\cov_medicated.mat', {'cov_medicated': cov_medicated.values})
    sio.savemat(r'D:\WorkStation_2018\SZ_classification\Data\fc_unmedicatedl.mat', {'fc_unmedicated': feature_firstepisodeunmed})
