...
class Utils():
	...
	def __init__(sel):
		...
		sel.test_data_path = r'D:\WorkStation_2018\WorkStation_CNN_Schizo\Data\ML_data_npy\dataset_206.npy'
		sel.n_node = n_node
	...
	def prep_data(sel):
			...
			"""
			# load data
			feature_train, label_train, feature_val, label_val, feature_test, label_test = sel.load_data()

			# encode label
			label_train = sel.gen_gradient_label(label_train)
			label_val = sel.gen_gradient_label(label_val)
			label_test = sel.gen_gradient_label(label_test)
			...
			feature_test = sel.trans_2d_to_4d(feature_test)
			...
			return feature_train, label_train, feature_val, label_val, feature_test, label_test
	...
	def load_data(sel):
		...
		data_test = np.load(sel.test_data_path)
		feature_test = data_test[:,1:]
		label_test = data_test[:,0]

		return feature_train, label_train, feature_val, label_val, feature_test, label_test

	def gen_gradient_label(sel, label):
		"""
...
class TrainTest(Utils):
	...
	def test_aftertrain(sel, model, data, label):
	    """
	...
	#    model.eval()  # eval mode (batchnorm uses moving mean/variance instead of mini-batch mean/variance)
	    with torch.no_grad():
	        ...
	        accuracy, sensitivity, specificity, auc_score = sel.eval_prformance(
	...
	def test_using_best_thresh(sel, model, thresh, data, label):
	    """
	    ...
	    accuracy, sensitivity, specificity, auc = sel.eval_prformance(label, predict, output.data)
	#    confusionmatrix = confusion_matrix(label, predict)
	    sel.plor_roc(label, output.data[:,1], is_savefig=is_savefig, fig_savepath=fig_savepath, fig_savename='ROC_test.tif')
...
if __name__ == '__main__':
    ...
    # prepare data
    feature_train, label_train, feature_val, label_val, feature_test, label_test = sel.prep_data() 
